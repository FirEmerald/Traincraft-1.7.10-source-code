		...
		if (cart1 instanceof AbstractTrains && cart2 instanceof AbstractTrains && ((AbstractTrains) cart2).isAttached && ((AbstractTrains) cart1).isAttached && areLinked(cart2, cart1)) {
			...
			if(d2>15){
				...
				if(cart1.worldObj!=null){
					...
					if(player!=null)
					{
						player.addChatMessage(new ChatComponentText("[TRAINCRAFT] The rolling stock at " + (int)cart1.posX + ", " + (int)cart1.posY + ", " + (int)cart1.posZ + " had a problem loading and has lost its link. Attached cart was too far away"));
					}
				}
