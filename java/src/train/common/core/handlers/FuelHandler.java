...
public class FuelHandler implements IFuelHandler {
	...
	public static int steamFuelLast(ItemStack it) {
		...
		{
			if (Block.getBlockFromItem(var1).getMaterial() == Material.wood) return wood;
			else if (Block.getBlockFromItem(var1) == Blocks.sapling) return sapling;
		}
		if (var1 == Items.stick)
			return stick;
		if (var1 == Items.coal)
		...
		if (var1 == Items.blaze_rod)
			return blazeRod;
		if (PluginIndustrialCraft.getItems().containsKey(PluginIndustrialCraft.getNames()[15]) && var1 == PluginIndustrialCraft.getItems().get(PluginIndustrialCraft.getNames()[15]).getItem())
			return coalFuel;
		if (PluginRailcraft.getItems().containsKey(PluginRailcraft.getNames()[1]) && var1 == PluginRailcraft.getItems().get(PluginRailcraft.getNames()[1]).getItem())
			return cokeCoal;
	...
	public int getBurnTime(ItemStack fuel) {
		...
		if(var1 == Item.getItemFromBlock(BlockData.oreTC.block) && (fuel.getItemDamage()==1 || fuel.getItemDamage()==2)){
			return 2400;
		}
		return 0;
	}
