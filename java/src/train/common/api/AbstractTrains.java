...
public abstract class AbstractTrains extends EntityMinecart implements IMinecart, IRoutableCart
{
	 ...
	 */
	public UUID trainOwner = UUID.fromString("000000000000-0000-0000-0000-00000000");

	public UUID getTrainOwner() {
		return trainOwner;
	}

	public void setTrainOwner(UUID owner) {
	 ...
	 */
	public UUID trainCreator = UUID.fromString("000000000000-0000-0000-0000-00000000");
	...
	public AbstractTrains(World world) {
		...
		dataWatcher.addObject(7, trainOwner.toString());
		...
		dataWatcher.addObject(13, trainCreator.toString());
		...
		for (EnumTrains trains : EnumTrains.values()) {
			if (trains.getEntityClass().equals(this.getClass())) {
				...
				ObfuscationReflectionHelper.setPrivateValue(EntityMinecart.class, this, trainSpec.getInternalName(), "field_94102_c", "entityName");
			}
		}
	}
			...
			if (!worldObj.isRemote)
			{
				...
				if (chunkTicket == null)
				{
					if (playerEntity != null && !this.chunkLoadErrorDisplayed)
					{
						...
						playerEntity.addChatMessage(new ChatComponentText("[TRAINCRAFT] The locomotive at " + (int) posX + ", " + (int) posY + ", " + (int) posZ + " will not load chunk because there are no more chunkloaders available"));
					}
					chunkForced = false;
				}
				else if(!chunkForced){
					chunkTicket.getModData().setInteger("locoID", this.getEntityId());
	...
	public void forceChunkLoading(Ticket ticket) {
		...
		if (this.playerEntity != null && !this.chunkLoadMsgDisplayed)
		{
			...
			playerEntity.addChatMessage(new ChatComponentText("[TRAINCRAFT] The locomotive at " + (int) posX + ", " + (int) posY + ", " + (int) posZ + " will keep chunks loaded for herself and " + (chunks.size()-1) + " carts"));
		}
	...
	protected void writeEntityToNBT(NBTTagCompound nbttagcompound) {
		...
		nbttagcompound.setString("theOwner", trainOwner.toString());
		nbttagcompound.setBoolean("Locked", locked);
		nbttagcompound.setString("theCreator", trainCreator.toString());
	...
	protected void readEntityFromNBT(NBTTagCompound nbttagcompound) {
		...
		trainOwner = UUID.fromString(nbttagcompound.getString("theOwner"));
		locked = nbttagcompound.getBoolean("Locked");
		setFlag(8, locked);
		trainCreator = UUID.fromString(nbttagcompound.getString("theCreator"));
	...
	protected void setUniqueIDToItem(ItemStack stack) {
		...
		stack.getTagCompound().setString("trainCreator", this.trainCreator.toString());
	 ...
	 */
	public void setTrainLockedFromPacket(boolean set, UUID owner) {
		locked = set;
		trainOwner = owner;
	}
	...
	protected boolean lockThisCart(ItemStack itemstack, EntityPlayer entityplayer) {
		if (itemstack != null && itemstack.getItem() instanceof IToolWrench) {
			if (entityplayer.getGameProfile().getId().equals(this.trainOwner)) {
				if (locked) {
					locked = false;
		...
		if (this.getTrainLockedFromPacket()) {
			if (damagesource.getEntity() instanceof EntityPlayer) {
				if (damagesource.getEntity() instanceof EntityPlayerMP)
				{
					...
					if (player.canCommandSenderUseCommand(2, "") && player.getHeldItem() != null && player.getHeldItem().getItem() instanceof IToolWrench)
					{
						((EntityPlayer) damagesource.getEntity()).addChatMessage(new ChatComponentText("Removing the train using OP permission"));
						return true;
					}
					else if (!player.getGameProfile().getId().equals(this.trainOwner))
					{
