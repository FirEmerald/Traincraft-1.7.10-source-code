...
public class TileEntityDistil extends TileEntity implements IInventory, IFluidHandler {
	...
	private boolean canSmelt() {
		...
		if (distilItemStacks[0].getItem() == Item.getItemFromBlock(BlockData.oreTC.block) && (distilItemStacks[0].getItemDamage() != 1 && distilItemStacks[0].getItemDamage() != 2)) {
			return false;
		}
	...
	private int getItemBurnTime(ItemStack it) {
		...
		if (var1 == Items.lava_bucket)
			return 20000;
		if (Block.getBlockFromItem(var1) == Blocks.sapling)
			return 100;
		if (var1 == Items.blaze_rod)
			return 2500;
		if (Block.getBlockFromItem(var1) == BlockData.oreTC.block && it.getItemDamage() == 1)
			return 2500;
		if (Block.getBlockFromItem(var1) == BlockData.oreTC.block && it.getItemDamage() == 2)
			return 2500;
		if (var1 == ItemData.diesel.item)
			return 4000;
		if (var1 == ItemData.refinedFuel.item)
			return 6000;
    ...
    public void onDataPacket(NetworkManager net, S35PacketUpdateTileEntity pkt)
    {
    	...
    	this.setFacing(ForgeDirection.VALID_DIRECTIONS[tag.getByte("f")]);
    	distilCookTime = tag.getShort("cookTime");
    	distilBurnTime = tag.getShort("burnTime");
    	amount = tag.getShort("amount");
    	liquidItemID = tag.getShort("liquid");
    }
