...
public class GuiFreight extends GuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 3) {
			if (player != null && player instanceof EntityPlayer && player.getGameProfile().getId().equals(((AbstractTrains) freight).trainOwner)) {
				if ((!((AbstractTrains) freight).locked)) {
	...
	protected void drawCreativeTabHoveringText(String str, int t, int g) {
		...
		fontRendererObj.drawStringWithShadow("Owner: " + ((AbstractTrains) freight).trainOwner.toString(), startX, startY + 40, -1);
	}
