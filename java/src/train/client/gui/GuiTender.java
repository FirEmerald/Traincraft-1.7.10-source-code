...
public class GuiTender extends GuiContainer {
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 3) {
			if(player!=null && player instanceof EntityPlayer && player.getGameProfile().getId().equals(((AbstractTrains) tender).trainOwner)){
				if ((!((AbstractTrains) tender).locked)){
	...
	protected void drawCreativeTabHoveringText(String str, int t, int g) {
		...
		fontRendererObj.drawStringWithShadow("Owner: "+((AbstractTrains) tender).trainOwner.toString(), startX, startY+40, -1);
	}
