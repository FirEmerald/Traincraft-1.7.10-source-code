...
public class GuiJukebox extends GuiScreen {
	...
	protected void actionPerformed(GuiButton button) {
		...
		if (button.id == 3) {
			if (player != null && player instanceof EntityPlayer && player.getGameProfile().getId().equals(((AbstractTrains) jukebox).trainOwner)) {
				if ((!((AbstractTrains) jukebox).locked)) {
	...
	protected void drawCreativeTabHoveringText(String str, int t, int g) {
		...
		fontRendererObj.drawStringWithShadow("Owner: " + ((AbstractTrains) jukebox).trainOwner.toString(), t + 15, g + 40 - 40, -1);
	}
